...
A = [data[2][3],'D','D',data[3][3]]
B = [data[2][5],'C','B',data[3][5]]
C = [data[2][7],'B','A',data[3][7]]
D = [data[2][9],'A','C',data[3][9]]
rooms = [A,B,C,D]
halls = ['','','','','','','']
...
def enter(rooms,halls,room,hall):
    ...
    if len(rooms[room])==4: return False,None,None,None
    pod = halls[hall]
    if room != ord(pod)-ord('A'): return False,None,None,None
    for p in rooms[room]:
        if p != pod: return False,None,None,None
    cost = 2
    if hall in [0,6]: cost -= 1
    cost += 3-len(rooms[room])
...
def check(rooms,halls,moves=[],c=0):
    ...
    if rooms == [ ['A','A','A','A'],['B','B','B','B'],['C','C','C','C'],['D','D','D','D'] ]: return True,0
